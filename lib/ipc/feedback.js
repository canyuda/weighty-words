...
function registerFeedbackIpc({ settingsStore, getActiveAnalyzer, getMainWindow }) {
  ...
  ipcMain.handle('get-final-report', async (event, { fullText, stats }) => {
    ...
    try {
      ...
      logger.info('feedback', '最终报告生成开始', { chars: fullText.length });
      ...
      logger.info('feedback', '最终报告生成完成', { chars: report.length });
      return { success: true, report };
    } catch (error) {
      logger.warn('feedback', '最终报告生成失败', { error: error.message });
      return { success: false, error: error.message };
    }
  });
}
