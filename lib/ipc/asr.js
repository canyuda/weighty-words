...
class ASRController {
  ...
  feed(samples) {
    if (this.session) {
      if (this.samples === 0) console.log('[asr] first audio chunk received');
      this.samples += samples.length;
      if (this.samples - (this.lastLoggedSamples || 0) >= 32000) { // ≈2s @16kHz
        console.log(`[asr] fed ${Math.round(this.samples / 16000)}s`);
...
function registerAsrIpc({ settingsStore, controller, usageTracker, getMainWindow, configureModelsDir, broadcastToWindows }) {
  ...
  ipcMain.handle('init-asr', async () => {
    try {
      ...
      console.log(`[asr] init-asr ok: engine=${session.name} fellBack=${!!session.fellBack}`);
      return {
        success: true,
        engine: session.name,
      ...
      console.log(`[asr] init-asr FAILED: ${payload.message} (category=${payload.category})`);
