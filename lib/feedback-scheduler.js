...
(function (root, factory) {
  ...
  function createFeedbackScheduler({ triggerChars = 30, windowChars = 500, send } = {}) {
    ...
    async function fire() {
      ...
      // INFO 锚点（渲染层经 console-message 转发落盘）：单字符串参数，
      // console-message 事件只保留格式化文本，对象参数会变 [object Object]
      console.log(`[feedback] 实时反馈触发 chars=${fullText.length} window=${windowChars} threshold=${threshold}`);
      try {
