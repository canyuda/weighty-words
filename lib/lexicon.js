...
function loadLexicon() {
  const lex = store.getWords();
  logger.info('lexicon', '词库加载完成', {
    fillers: lex.fillers.length,
    hedges: lex.hedges.length,
    vague: Object.keys(lex.vague).length,
    emotions: Object.keys(lex.emotions).length
  });
  const err = [store.getFactoryError(), store.getError()].filter(Boolean).join('\n');
  if (err) logger.warn('lexicon', err);
}
