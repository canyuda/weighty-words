...
async function callLLM(config, prompt, maxTokens, temperature, timeoutMs, onDelta) {
  ...
  // INFO 起止行只带元数据（结构上无密钥）；DEBUG 才记全量请求/响应，
  // 且请求头过 redactHeaders、body 过 scrubber（logger 内 compactJson 统一处理）
  const startedAt = Date.now();
  const startMeta = { stream };
  if (timeoutMs) startMeta.timeoutSec = Math.round(timeoutMs / 1000);
  logger.info('llm', `→ POST ${req.url}`, startMeta);
  logger.debug('llm', 'request', { url: req.url, headers: redactHeaders(req.headers), body: req.body });
  ...
  try {
    ...
    const message = isAbort
      ...
      : `网络请求失败: ${e.message}`;
    logger.error('llm', message, { durationMs: Date.now() - startedAt });
    throw new Error(message);
  }
  // Response body (SSE 全文或整包) 一处统一记 DEBUG：成功与后续解析失败共用
  logger.debug('llm', 'response', { status: response.status, body: streamedFull !== null ? streamedFull : rawText });
  logger.info('llm', `← ${response.status} ${rawText.length}B${streamedFull !== null ? ' (streamed)' : ''}`, { durationMs: Date.now() - startedAt });
  if (streamedFull !== null) {
    // SSE 消费完成：增量已回调，累积全文即结果（无需 JSON 解析）
    return streamedFull;
  }
  const logErrorAndThrow = (message, errorLabel) => {
    logger.error('llm', message, { reason: errorLabel, durationMs: Date.now() - startedAt });
    throw new Error(message);
  };
  if (!response.ok) {
    logErrorAndThrow(
...
module.exports = { sendFeedback, sendReport, testConnection, resolveAnalyzerConfig, extractApiErrorMessage };
