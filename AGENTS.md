...
- **主进程** `main.js` + `lib/`：唯一主窗口 + 单窗口页面路由（工作台/设置/词库编辑器/规则定制四个页面在主窗内切换，不再创建二级 BrowserWindow）、全部 IPC handler、设置持久化。
  ...
  - `lib/config-paths.js`：`~/.weighty-words/` 配置目录唯一解析点（`EXPRESSION_TRAINER_CONFIG_DIR` 测试钩子优先）+ `ensureConfigDir` 幂等首启创建 + `atomicWriteFileSync` 原子写（settings/rules/words 三文件写入必须走它）+ `getLogsPath()`（`lib/logger.js` 的日志目录，随同一钩子隔离）。
  - `lib/logger.js`：统一分级文件日志，唯一写者 = 主进程。五级 `trace/debug/info/warn/error` 默认 `info`，级别唯一入口 = `settings.json` 的 `logging.level`（缺失补默认、启动读取一次重启生效、不做 UI），`--dev` 启动且级别仍为默认时自动升 DEBUG。单行不变式（`本地时间戳 [级别] [tag] 消息 key=value`，一行 = 一条），tag 注册表：app/settings/rules/lexicon/asr/llm/media/feedback/download/usage/renderer；文件 `~/.weighty-words/logs/app-YYYYMMDD-<序号>.log`（本地日期切分，写入量预估超 200MB 序号加一开新卷，30 天前旧卷按文件名日期在启动/跨天时自动清理），每条同步镜像 stdout（终端 + e2e grep 依赖）。脱敏双层：INFO 只记元数据（结构上无密钥），DEBUG 全量过凭据头脱敏 + key 名递归 scrubber；语音转写原文与 LLM 请求/响应正文仅 DEBUG 落盘。写失败静默降级 stdout 并只提示一次（全应用唯一吞异常豁免点）。渲染层 console 经 `console-message` 按级别转发落盘（error→ERROR / warning→WARN / 其余→INFO，未识别级别 INFO 兜底；消息换行由 logger 单行化），渲染进程零改动。
...
- **测试与脚本约定**：e2e 验证脚本（tests/e2e/ 下 smoke 与 verify-*）启动 Electron 时 MUST 设 `EXPRESSION_TRAINER_AUTOMATION=1`（跳过关闭确认等需人工交互的阻塞点）、`EXPRESSION_TRAINER_USER_DATA=/tmp/<专属目录>`（隔离 userData——用量簿记与模型缓存）与 `EXPRESSION_TRAINER_CONFIG_DIR=<同目录>`（隔离 `~/.weighty-words/` 用户配置，自动化绝不可触碰真实配置目录）。**常驻埋点**（统一经 `lib/logger.js` 落盘 + stdout 镜像，`npm start` 一个终端看全两个进程，排障先看 `~/.weighty-words/logs/` 当日文件）：渲染层 `[media]`（app.js 全链路：选文件→解码→播放→喂入→ASR 结果，经 console-message 转发落盘）、主进程 `[asr]`（init 结果）、`[llm]`（调用起止/耗时/DEBUG 全量）、`[feedback]`（实时反馈触发/报告起止）、`[download]`（模型下载）等域锚点；请求/响应正文与转写原文仅 DEBUG 级落盘，默认 INFO 只记元数据。`scripts/`：asr-connectivity（云端 ASR 连通性验证，凭据只走环境变量）、generate-icon + fix-dev-icon（图标重生成与开发模式 Dock 闪图标修复，后者挂 postinstall）。
